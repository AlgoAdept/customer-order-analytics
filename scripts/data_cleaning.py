...
BRONZE_PATH = Path("data/bronze/SuperStoreOrders.csv")
SILVER_PATH = Path("data/silver/clean_superstore_orders.csv")
...
def validate_data(df: pd.DataFrame):
    ...
    if invalid_sales > 0:
        raise ValueError(
            f"{invalid_sales} invalid sales values found."
        )
...
def save_data(df: pd.DataFrame, output_path: Path):
    ...
    logger.info(
        f"Silver dataset saved successfully:\n{output_path}"
    )
